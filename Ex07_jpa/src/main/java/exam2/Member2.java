...
public class Member2 {
	...
	@SequenceGenerator (
			name="mySequence",
			sequenceName = "JpaMember2_seq", // 실제 DB에 생성되는 시퀸스 이름
			initialValue = 1,		// 초기값
			allocationSize = 1		// 1씩증가
	)
	@GeneratedValue(generator = "mySequence") // 시퀸스의 별칭	
	private Long id;
	...
	@Transient
	private long timestamp;		// 어노테이션을 이용하여 영속대상(DB)에서 제외
	transient private long timestamp2;	// 키워드를 이용하여 영속대상(DB)에서 제외
	
	public Member2() {
		
	}
	public Member2(String password) {
